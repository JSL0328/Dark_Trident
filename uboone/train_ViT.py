...
def TrainViT():
    ...
    fout = open(output_dir + 'ViT_training_metrics_{}_{}.csv'.format(timestr(), title), 'w')
    fout.write('train_accu,test_accu,train_loss,test_loss,epoch,step')
    fout.write('\n')

    ViT_weights = weights_dir + "ViT_model_{}_epoch_{}_batch_id_{}_labels_{}_title_{}_step_{}.pwf"
    ...
    vit = mpid_net_binary.ViT(dropout=cfg.drop_out, num_classes=2)
    vit.cuda()

    loss_fn = nn.BCEWithLogitsLoss()

    optimizer = optim.Adam(vit.parameters(), lr=cfg.learning_rate)
    train_step = mpid_func_binary.make_train_step(vit, loss_fn, optimizer)
    test_step = mpid_func_binary.make_test_step(vit, test_loader, loss_fn, optimizer)
        ...
        for batch_idx, (x_batch, y_batch, info_batch, nevents_batch) in enumerate(train_loader):
            ...
            if (batch_idx % cfg.test_every_step == 1 and cfg.run_test):
                if (cfg.save_weights and epoch >= 3 and epoch <= 6):
                    torch.save(vit.state_dict(), ViT_weights.format(timestr(), epoch, batch_idx, labels, title, step))

                print("Start eval on test sample.......@step..{}..@epoch..{}..@batch..{}".format(step, epoch, batch_idx))
                test_accuracy = mpid_func_binary.validation(vit, test_loader, cfg.batch_size_test, train_device, event_nums=cfg.test_events_nums)
                print("Test Accuracy {}".format(test_accuracy))
                print("Start eval on training sample...@epoch..{}.@batch..{}".format(epoch, batch_idx))
                train_accuracy = mpid_func_binary.validation(vit, train_loader, cfg.batch_size_train, train_device, event_nums=cfg.test_events_nums)
